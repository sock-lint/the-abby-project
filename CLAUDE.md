...
frontend/src/
  ...
  components/
    ...
    dashboard/       HeroPrimaryCard (role-aware context-picking hero for
                     ...
                     KidStrip, WeekGlanceBlock, QuickAdjustRow.
...
- **Dashboard (role-split):** `frontend/src/pages/Dashboard.jsx` is a thin router — loads `/api/dashboard/` once and branches to `ChildDashboard` or `ParentDashboard` based on `user.role`. The shared loader + error-retry block lives at this layer so integration tests can exercise both roles from one entry point. **Child** body: contextual `HeroPrimaryCard` (priority `clocked → streak-risk → next-chore → quest-progress → idle`), horizontal `VitalPipStrip` (coins/streak/level/pet), Today's Log capped at 5 items with expander, loot rail, and everything else wrapped in `AccordionSection` (collapsed by default, open state persisted per-title in `localStorage` keyed `dashboard-accordion-{slug}`). **Parent** body: `useParentDashboard` hook aggregates pending chores + pending homework submissions + pending redemptions in parallel via `Promise.all`, unifies to `{ id, kind, kidId, kidName, title, subtitle, reward, submittedAt }`, sorts newest-first; `ApprovalQueueList` groups by kid with inline Approve/Reject rows that call the existing `approveChoreCompletion` / `approveHomeworkSubmission` / `approveRedemption` endpoints and optimistically remove the row. Today's Log is retired for parents. No backend changes — the parent dashboard is purely a frontend aggregation.
- **Global header + FAB:** `JournalShell` composes avatar (mobile) + `HeaderStatusPips` (middle, role-aware) + `NotificationBell` (right), followed by `HeaderProgressBand` — a 2–3px full-width band that's inert by default and renders a sheikah-teal gradient scaled to `getActiveQuest().progress_percent` when a quest is active. `QuickActionsFab` replaces the old `ClockFab` — single bottom-right button that opens `QuickActionsSheet` with role-aware contextual actions: child sees Clock in · Add homework · Submit due (only when a due assignment exists) · Start a quest (only when a quest scroll is in inventory) · Request a reward · Contribute to savings goal (only when an active goal exists); parent sees Clock in · Create homework for a kid · Adjust coins · Adjust payment. Clock-in is still the most common action and is the default focus when opened. The FAB continues to show a running-timer chip while clocked in so the at-a-glance signal is preserved.
