...
- **Co-parent invite links**: `POST /api/family/invites/` (parent-only) mints a single-use, 24h `FamilyInvite` token; the anonymous `/join/<token>` page previews the family via `GET /api/auth/join/<token>/` and redeems via POST (same `{token, user, family}` shape as signup, shares the `signup` throttle scope). Redemption is atomic under `select_for_update`, role is hard-coded to parent, and unknown/expired/used tokens all return one generic 404 so callers can't probe which. Deliberately NOT gated on `ALLOW_PARENT_SIGNUP` — invites are parent-initiated. See [apps/families/views.py](apps/families/views.py) + `FamilyService.create_parent_from_invite`; tests in [`apps/families/tests/test_invites.py`](apps/families/tests/test_invites.py). Frontend: "Invite by link" in `Manage → Family` + [`pages/Join.jsx`](frontend/src/pages/Join.jsx).
