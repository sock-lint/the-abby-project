...
- **Notifications:** `apps.projects.Notification` with 11 `NotificationType`s (incl. `redemption_requested`, `chore_submitted`, `chore_approved`). Routes under `/api/notifications/` with `unread_count`, `mark_all_read`, and per-item `mark_read` actions.
