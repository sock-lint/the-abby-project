...
apps/
  ...
  achievements/      SkillCategory → Subject → Skill → Badge (full
                     skill-tree hierarchy, 20 criterion types incl.
...
- `BASE_DROP_RATES` — dict of trigger → base drop probability (clock_out 0.40, chore 0.30, homework_complete 0.35, homework_created 0.15, milestone 0.80, badge 1.00, quest_complete 1.00, perfect_day 1.00, habit_log 0.15). `homework_created` is additionally daily-capped in `HomeworkService.create_assignment` — second-and-later creates per day pass `drops_allowed=False` so only streak + quest progress fire.
