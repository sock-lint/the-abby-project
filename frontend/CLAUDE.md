...
The illuminated-manuscript vocabulary the Skills page introduced now lives at the shared layer. Six primitives + the `mastery.constants` module:
...
- `<FolioHeadband tierKey />` — the woven cloth cap across the top of an open folio's verso (`QuestFolio`, `FolioSpread`), tier-tinted to match the `TomeSpine` it opens from. Decorative; carries `data-tier`. Both terminals stay inboard of the card corners so they clear `ParchmentCard`'s flourishes and the `rounded-xl` clip, and `.folio-headband` masks the trailing end below `md` where the verso becomes a full-width banner.
...
- **CSS contract** (declared once in [`index.css`](src/index.css), reused everywhere): `.versal-gilt` with `--versal-fill` custom property; `.folio-headband`; `@keyframes halo-rise` / `gilded-glint`; `--color-rarity-{common,uncommon,rare,epic,legendary}`. All animations sit inside the existing `prefers-reduced-motion` block.
