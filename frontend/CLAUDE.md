...
- **Offline read-only caching** (2026-06): `vite.config.js` workbox `runtimeCaching` has a single GET-only `NetworkFirst` rule (`api-read-cache`, 4s network timeout, 24h/64-entry expiry) over `/api/(dashboard|chores|homework|inventory)/` so those surfaces render from the last saved copy when offline; writes are never cached and fail loudly. **`OfflineBanner`** ([`pwa/OfflineBanner.jsx`](src/pwa/OfflineBanner.jsx)), mounted next to `UpdateBanner` in `App.jsx`, listens to the window online/offline events and explains the stale-copy state.
