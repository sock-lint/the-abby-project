...
- `pages/bestiary/codex/BestiaryFolio.jsx` + `codex.constants.js` + `SpeciesTile.jsx` + `SpeciesDetailSheet.jsx` — used only by `pages/bestiary/codex/BestiaryCodex.jsx`. Mirrors the Reliquary's `SigilCodex` + `CollectionFolio` + `collections.constants.js` shape applied to the Bestiary (chapter taxonomy: Bound in Lore / Bonded / Hatched / Silhouettes — bucketed by progress against each species's `available_potions` set). Kept page-scoped because the chapter predicates are Bestiary-domain — see CLAUDE.md → "Bestiary codex alignment"
...
Use these primitives whenever a surface is meant to feel like an authored chapter rather than a form. Current consumers: the Skills tome shelf, the Reliquary Codex, the Sigil Frontispiece, the Lorebook folio, Bestiary (all four sub-tabs wear `IncipitBand` heroes; Companions/Mounts use `TomeShelf variant="vessel"` for filter pills; the Codex is a full `IncipitBand` → `TomeShelf` → `BestiaryFolio` shape mirroring SigilCodex), Project detail (status-driven versal + milestone numerals), Yearbook chapter cards (current-year IncipitBand, past-year small versal), and the dashboard hero (versal on clocked / next-action / quest-progress variants + AccordionSection chapter numerals).
...
| `mastery.constants.js` | `PROGRESS_TIER`, `TIER_BADGE_TONE`, `tierForProgress`, `RARITY_HALO`, `CHAPTER_NUMERALS`, `chapterMark`, `countIlluminated`, `RECENT_EARNED_DAYS`, `isRecentlyEarned`, `RARITY_KEYS`, `RARITY_ORDER` | Atlas vocabulary. `tierForProgress({ unlocked, progressPct, level, maxLevel })` resolves to one of `PROGRESS_TIER` (`locked` / `nascent` / `rising` / `cresting` / `gilded`) — the same five tiers drive `IlluminatedVersal` fill color, `TomeSpine` foot-bands, and `SkillVerse` level straps. `RARITY_HALO[rarity]` is the canonical ring + glow for any earned thing |

**Tier is expressed through `PROGRESS_TIER`'s two channels — `bar` and `chip` — never as applied ornament.** A folio wears its tier as the versal's gilt fill, the progress bar, the stat values, and a `RuneBadge` on its kicker (`TIER_BADGE_TONE` maps the five rungs onto the badge's `ink` / `moss` / `teal` / `ember` / `gold` tones). This matches `QuestLogEntry`, which signals all four of its states purely as surface + border tint, and `SkillVerse`, which tints the versal and level numeral. A tier-tinted band was tried on the folio verso and removed: it was the only element in the app encoding state as a decorative band, its `--color-headband-*` hexes were the only tier colors not resolving through cover tokens, and it restated a tier the card was already showing three other ways. `TomeSpine` keeps its headband — there the band depicts a physical object (the cloth cap on a binding), which is a different job from signalling state.

CSS contract (declared once in [`../index.css`](../index.css), reused by every cohort consumer):

- `.drawer-case` + `.drawer-face` / `.drawer-face-open` — the `vessel` variant's cabinet. **`codex` and `vessel` must not share their physical vocabulary.** They deliberately share outer dimensions (so a mixed shelf keeps its snap-rail layout), which means everything else has to carry the distinction: a tome rests **on** a `.shelf-board` and, when active, lifts, tilts open on its binding, and unfurls a bookmark ribbon; a drawer sits **in** a `.drawer-case`, is debossed into it while shut, and when active slides out toward the viewer with its pull gripped — no rotation, and **no ribbon**, since a drawer has no bookmark. Sharing the ribbon and the plank across variants was what made a row of drawers read as a row of books
