...
| `FolioHeadband.jsx` | `<FolioHeadband tierKey className />` | The woven cloth cap across the top of an open folio's verso, tier-tinted to match the `TomeSpine` it opens from. Decorative (`aria-hidden`), exposes `data-tier`. Deliberately ends inboard of both card corners: `ParchmentCard`'s flourishes sit at `top-1 left-1` and a full-bleed band buries their outer stroke and gets sliced into a wedge by the `rounded-xl` + `overflow-hidden` clip. Pairs with the `.folio-headband` mask (see CSS contract) |
...
- `.folio-headband` — trailing-end mask for `<FolioHeadband>`. Below `md` a folio's verso is a full-width banner rather than a 220-260px column, and a hard-edged 6px band drawn the whole way across reads as chrome (a stalled progress bar) however it is textured; the mask lets it read as cloth running off toward the binding. Removed at `md+`, where the band already terminates at the gutter fold
