...
class StreakServiceTests(TestCase):
    ...
    def test_daily_check_in_bonus_coins(self):
        result = StreakService.record_activity(self.user, activity_date=date(2026, 4, 1))
        # streak=1, multiplier = 1 + 1*0.10 = 1.10, bonus = int(5 * 1.10) = 5
    ...
    def test_daily_check_in_bonus_scales_with_streak(self):
        ...
        # streak=11, multiplier = 1 + 11*0.10 = 2.10, bonus = int(5 * 2.10) = 10
        multiplier = min(1 + 11 * 0.10, 3.0)
        expected = int(5 * multiplier)
        self.assertEqual(expected, 10)

    def test_daily_check_in_bonus_caps_at_day_20(self):
        """Once streak hits the cap (3.0x at day 20), daily bonus plateaus at 15 coins."""
        start = date(2026, 4, 1)
        for i in range(20):
            StreakService.record_activity(self.user, activity_date=start + timedelta(days=i))

        profile = CharacterProfile.objects.get(user=self.user)
        self.assertEqual(profile.login_streak, 20)
        # Compute: multiplier = min(1 + 20*0.10, 3.0) = min(3.0, 3.0) = 3.0
        # bonus = int(5 * 3.0) = 15
        multiplier = min(1 + profile.login_streak * 0.10, 3.0)
        expected = int(5 * multiplier)
